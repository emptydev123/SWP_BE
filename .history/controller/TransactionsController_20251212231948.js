...
/**
 * TransactionsController - Xử lý các giao dịch thanh toán PayOS
 * 
 * Chức năng:
 * 1. Tạo payment link chung cho tất cả loại thanh toán (MEMBERSHIP, EVENT_TICKET)
 * 2. Xử lý webhook từ PayOS để cập nhật trạng thái transaction
 * 3. Xử lý return/cancel URL từ PayOS
 */

/**
 * Tạo payment link chung - hỗ trợ cả MEMBERSHIP và EVENT_TICKET
 * @param {string} type - Loại transaction: 'MEMBERSHIP' hoặc 'EVENT_TICKET'
 * @param {string} membershipId - ID membership (nếu type = MEMBERSHIP)
 * @param {string} eventId - ID event (nếu type = EVENT_TICKET)
 * @param {string} ticketType - Loại vé (nếu type = EVENT_TICKET)
 * @param {number} quantity - Số lượng vé (nếu type = EVENT_TICKET)
 */
exports.createPayment = async (req, res) => {
    try {
        const { type, membershipId, eventId, ticketType, quantity = 1 } = req.body;
        ...
        // 2. Xử lý theo type
        if (type === 'MEMBERSHIP') {
            return await handleMembershipPayment(req, res, membershipId, userId);
 ...
 */
async function handleMembershipPayment(req, res, membershipId, userId) {
    try {
        ...
        if (!membershipId) {
            return res.status(400).json({
                success: false,
                message: 'Thiếu membershipId'
            });
        }

        // 2. Lấy thông tin membership và club
        ...
        // 3. Kiểm tra quyền: chỉ user sở hữu membership mới được thanh toán
        if (membership.userId !== userId) {
            return res.status(403).json({
                success: false,
                message: 'Không có quyền thanh toán cho membership này'
            });
        }

        // 4. Kiểm tra club có bật phí gia nhập không
        if (!membership.club.membershipFeeEnabled || membership.club.membershipFeeAmount <= 0) {
            return res.status(400).json({
                success: false,
        ...
        // 5. Kiểm tra membership status - chỉ cho phép thanh toán nếu đang PENDING_PAYMENT
        if (membership.status !== 'PENDING_PAYMENT') {
            return res.status(400).json({
                success: false,
                message: `Membership đang ở trạng thái ${membership.status}, không thể thanh toán`
            });
        }

        // 6. Kiểm tra xem đã có transaction PENDING chưa
        const existingTransaction = await prisma.transaction.findFirst({
            where: {
                referenceMembershipId: membershipId,
                status: 'PENDING',
                type: 'MEMBERSHIP'
            }
        });
        ...
        // 7. Tạo transaction mới trong DB
...
/**
 * Tạo payment link cho mua vé event
 * - Khi sinh viên muốn mua vé cho event có tính phí
 * - Tạo Ticket với status RESERVED
 * - Tạo Transaction với type EVENT_TICKET
 * - Tạo payment link từ PayOS
 */
exports.createEventTicketPayment = async (req, res) => {
    try {
